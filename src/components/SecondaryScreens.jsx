...
function Header({ title, onBack, onLog, currentScreen }) {
  return (
    <div style={{ background:'#202c33', padding:'0 16px', display:'flex', alignItems:'center', gap:12, minHeight:60, flexShrink:0 }}>
      ...
      <span style={{ color:'#e9edef', fontSize:16, fontWeight:600 }}>{title}</span>
    </div>
  );
}
...
export function ContactInfo({ chat, onNavigate, onLog }) {
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:'#111b21', overflow:'hidden' }}>
      <Header title="Contact Info" onBack={() => onNavigate(SCREENS.CHAT_VIEW)} onLog={onLog} currentScreen={currentScreen} />
      <div style={{ flex:1, overflowY:'auto' }}>
        {/* Profile */}
        <div style={{ background:'#202c33', padding:'32px 24px', textAlign:'center', marginBottom:8 }}>
          <div style={{ width:100, height:100, borderRadius:'50%', background:contact.color+'33', border:`3px solid ${contact.color}66`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:32, fontWeight:700, color:contact.color, margin:'0 auto 16px' }}>{contact.avatar}</div>
          <div style={{ color:'#e9edef', fontSize:22, fontWeight:600 }}>{contact.name}</div>
          <div style={{ color:'#8696a0', fontSize:14, marginTop:4 }}>{contact.phone}</div>
        </div>

        {/* Actions */}
        <div style={{ background:'#202c33', margin:'0 0 8px', padding:'8px 0' }}>
          {[
            ...
            <div key={item.label} onClick={() => logTap(item.target, item.label)}
              style={{ display:'flex', alignItems:'center', gap:16, padding:'14px 24px', cursor:'pointer', color:'#e9edef', fontSize:15 }}
        ...
        {/* Media tab */}
        <div style={{ background:'#202c33', padding:'16px 24px' }}>
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:'#111b21', overflow:'hidden' }}>
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'12px 0' }}>
        {starred.length === 0 ? (
          <div style={{ textAlign:'center', color:'#8696a0', fontSize:14, padding:'60px 20px' }}>
          ...
          <div key={m.id} style={{ background:'#202c33', margin:'4px 12px', borderRadius:10, padding:'14px 16px' }}>
            <div style={{ color:'#00a884', fontSize:12, fontWeight:600, marginBottom:6 }}>{m.contactName}</div>
            <div style={{ color:'#e9edef', fontSize:14 }}>{m.text}</div>
          </div>
        ))}
      </div>
    </div>
...
export function Settings({ onNavigate, onLog }) {
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:'#111b21', overflow:'hidden' }}>
      ...
      <div style={{ flex:1, overflowY:'auto' }}>
        <div style={{ background:'#202c33', margin:'8px 0', padding:'8px 0' }}>
          {['Account', 'Privacy', 'Security', 'Notifications', 'Storage and data', 'App language', 'Help', 'About'].map(item => (
            <div key={item} onClick={() => onLog({ screen_id:SCREENS.SETTINGS, action_type:'tap', target_id:`TGT_SETTING_${item.replace(/ /g,'_').toUpperCase()}`, target_label:item })}
              style={{ padding:'14px 24px', color:'#e9edef', fontSize:15, cursor:'pointer', borderBottom:'1px solid #1a2530' }}
              onMouseEnter={e => e.currentTarget.style.background='#2a3942'}
              onMouseLeave={e => e.currentTarget.style.background='transparent'}>
              {item}
            </div>
...
export function EmptyState() {
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', background:'#0b141a' }}>
      <div style={{ opacity:0.08, marginBottom:24 }}>
        <svg width="120" height="120" viewBox="0 0 24 24" fill="white"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.125.557 4.126 1.532 5.86L.057 23.516a.75.75 0 00.927.927l5.656-1.475A11.943 11.943 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0z"/></svg>
      </div>
      <div style={{ color:'#e9edef', fontSize:22, fontWeight:300, marginBottom:8 }}>WhatsApp Web</div>
      <div style={{ color:'#8696a0', fontSize:14, textAlign:'center', maxWidth:380, lineHeight:1.6 }}>
