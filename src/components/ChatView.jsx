...
export default function ChatView({ chat, onNavigate, onLog, onSend, onForward, onStar, onViewMessage, taskState, updateTaskState }) {
  ...
  return (
    <div style={{ flex:1, display:'flex', flexDirection:'column', background:'#0b141a', position:'relative', overflow:'hidden' }}>
      ...
      <div style={{ background:'#202c33', padding:'0 16px', display:'flex', alignItems:'center', gap:12, minHeight:60, zIndex:10, boxShadow:'0 2px 8px rgba(0,0,0,0.3)' }}>
        <button onClick={handleBack} style={{ background:'none', border:'none', color:'#00a884', cursor:'pointer', fontSize:22, padding:'4px 8px 4px 0', lineHeight:1 }}>←</button>
        <div onClick={handleContactHeader} style={{ display:'flex', alignItems:'center', gap:12, cursor:'pointer', flex:1 }}>
          <Avatar contact={contact} />
          <div>
            <div style={{ color:'#e9edef', fontSize:15, fontWeight:600 }}>{contact.name}</div>
            <div style={{ color:'#8696a0', fontSize:13 }}>online</div>
          </div>
        </div>
        <div style={{ display:'flex', gap:4 }}>
          <IconBtn title="Video call"><svg width="20" height="20" viewBox="0 0 24 24" fill="#aebac1"><path d="M17 10.5V7c0-.55-.45-1-1-1H4c-.55 0-1 .45-1 1v10c0 .55.45 1 1 1h12c.55 0 1-.45 1-1v-3.5l4 4v-11l-4 4z"/></svg></IconBtn>
          <IconBtn title="Search in chat" onClick={() => onNavigate(SCREENS.SEARCH)}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#aebac1" strokeWidth="2" strokeLinecap="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg></IconBtn>
          <IconBtn title="More options"><svg width="20" height="20" viewBox="0 0 24 24" fill="#aebac1"><circle cx="12" cy="5" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="19" r="1.5"/></svg></IconBtn>
        </div>
      </div>
          ...
          return (
            <div key={msg.id} style={{ display:'flex', justifyContent: isMe ? 'flex-end' : 'flex-start', marginBottom:4, position:'relative' }}
              ...
              <div onClick={() => handleMsgClick(msg)} style={{ maxWidth:'65%', cursor:'default' }}>
                {/* Reply preview */}
                {replyMsg && (
                  <div style={{ background: isMe ? '#1d6a50' : '#2a3942', borderLeft:'4px solid #00a884', borderRadius:'4px 4px 0 0', padding:'6px 10px', fontSize:12, color:'#8696a0', marginBottom:0 }}>
                ...
                <div style={{
                  ...
                  padding:'8px 12px 6px', boxShadow:'0 1px 2px rgba(0,0,0,0.3)',
                  border: msg.starred ? '1px solid #f0b42966' : 'none',
                }}>
                  {msg.forwarded && <div style={{ color:'#8696a0', fontSize:11, marginBottom:3, display:'flex', alignItems:'center', gap:4 }}>↪ Forwarded</div>}
                  <span style={{ color:'#e9edef', fontSize:14, lineHeight:1.5, wordBreak:'break-word' }}>{msg.text}</span>
                  <div style={{ display:'flex', justifyContent:'flex-end', alignItems:'center', gap:4, marginTop:2 }}>
                    {msg.starred && <span style={{ fontSize:10 }}>⭐</span>}
                    <span style={{ color:'#8696a0', fontSize:11 }}>{formatTime(msg.time)}</span>
              ...
              {menuOpen && (
                <div style={{ position:'absolute', [isMe ? 'right' : 'left']:0, top:'100%', background:'#233138', borderRadius:8, boxShadow:'0 8px 24px rgba(0,0,0,0.5)', zIndex:50, minWidth:180, overflow:'hidden' }}>
                  {[
                    ...
                    <div key={item.label} onClick={item.action}
                      style={{ padding:'12px 16px', color:'#e9edef', fontSize:14, cursor:'pointer' }}
      ...
      {replyTo && (
        <div style={{ background:'#202c33', borderTop:'1px solid #2a3942', padding:'8px 16px', display:'flex', alignItems:'center', gap:12, zIndex:10 }}>
          <div style={{ flex:1, borderLeft:'4px solid #00a884', paddingLeft:12 }}>
            ...
            <div style={{ color:'#8696a0', fontSize:13 }}>{replyTo.text.slice(0,60)}</div>
          </div>
          <button onClick={() => { setReplyTo(null); onLog({ screen_id:SCREENS.CHAT_VIEW, action_type:'tap', target_id:TARGETS.REPLY_CANCEL, target_label:'cancel reply' }); }}
            style={{ background:'none', border:'none', color:'#8696a0', cursor:'pointer', fontSize:20, lineHeight:1 }}>×</button>
        </div>
      )}

      <div style={{ background:'#202c33', padding:'8px 16px', display:'flex', alignItems:'center', gap:10, zIndex:10 }}>
        <IconBtn title="Emoji" onClick={() => onLog({ screen_id:SCREENS.CHAT_VIEW, action_type:'tap', target_id:TARGETS.EMOJI_BTN, target_label:'emoji' })}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="#8696a0"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 18c-4.41 0-8-3.59-8-8s3.59-8 8-8 8 3.59 8 8-3.59 8-8 8zm3.5-9c.83 0 1.5-.67 1.5-1.5S16.33 8 15.5 8 14 8.67 14 9.5s.67 1.5 1.5 1.5zm-7 0c.83 0 1.5-.67 1.5-1.5S9.33 8 8.5 8 7 8.67 7 9.5 7.67 11 8.5 11zm3.5 6.5c2.33 0 4.31-1.46 5.11-3.5H6.89c.8 2.04 2.78 3.5 5.11 3.5z"/></svg>
        </IconBtn>
        <IconBtn title="Attach" onClick={() => onLog({ screen_id:SCREENS.CHAT_VIEW, action_type:'tap', target_id:TARGETS.ATTACH_BTN, target_label:'attach' })}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="#8696a0"><path d="M16.5 6v11.5c0 2.21-1.79 4-4 4s-4-1.79-4-4V5c0-1.38 1.12-2.5 2.5-2.5s2.5 1.12 2.5 2.5v10.5c0 .55-.45 1-1 1s-1-.45-1-1V6H10v9.5c0 1.38 1.12 2.5 2.5 2.5s2.5-1.12 2.5-2.5V5c0-2.21-1.79-4-4-4S7 2.79 7 5v12.5c0 3.04 2.46 5.5 5.5 5.5s5.5-2.46 5.5-5.5V6h-1.5z"/></svg>
        </IconBtn>
        <input
          ref={inputRef}
          value={input}
          ...
          style={{ flex:1, background:'#2a3942', border:'none', borderRadius:8, padding:'11px 16px', color:'#e9edef', fontSize:14, outline:'none', fontFamily:'inherit' }}
        />
      ...
      {showForward && (
        <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,0.7)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center' }}>
          <div style={{ background:'#202c33', borderRadius:12, width:380, maxHeight:'70vh', overflow:'hidden', display:'flex', flexDirection:'column' }}>
            <div style={{ padding:'16px 20px', borderBottom:'1px solid #2a3942', display:'flex', alignItems:'center', gap:12 }}>
              <button onClick={() => { setShowForward(null); onLog({ screen_id:SCREENS.CHAT_VIEW, action_type:'tap', target_id:TARGETS.FORWARD_CANCEL, target_label:'cancel forward' }); }}
                style={{ background:'none', border:'none', color:'#8696a0', cursor:'pointer', fontSize:20 }}>×</button>
              <div>
                <div style={{ color:'#e9edef', fontSize:16, fontWeight:600 }}>Forward message</div>
                <div style={{ color:'#8696a0', fontSize:12 }}>Select a contact</div>
              </div>
            </div>
            <div style={{ padding:'10px 16px', borderBottom:'1px solid #2a3942', background:'#1a2530', fontSize:13, color:'#8696a0', fontStyle:'italic' }}>
            ...
            <div style={{ overflowY:'auto' }}>
              {CONTACTS.filter(c => c.id !== chat.contactId).map(c => (
                <div key={c.id} onClick={() => handleForwardSend(c)}
                  ...
                  <span style={{ color:'#e9edef', fontSize:15 }}>{c.name}</span>
                </div>
              ))}
            </div>
          </div>
...
function IconBtn({ children, onClick, title }) {
  ...
  return (
    <button onClick={onClick} title={title} onMouseEnter={() => setH(true)} onMouseLeave={() => setH(false)}
      style={{ background: h ? '#2a3942' : 'none', border:'none', borderRadius:'50%', width:40, height:40, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', flexShrink:0 }}>
      {children}
    </button>
  );
}
...
function MsgActionBtn({ children, onClick, title }) {
  return (
    <button onClick={onClick} title={title}
      style={{ background:'#233138', border:'none', borderRadius:'50%', width:30, height:30, display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', color:'#aebac1', fontSize:16, boxShadow:'0 2px 6px rgba(0,0,0,0.3)' }}>
      {children}
    </button>
  );
}